...
- La troisième et dernière étape de #13 est livrée : le simulateur IFI partage désormais `src/change.js` — `exchangerate-api.com` et le repli non daté `FX_FALLBACK` sont retirés. #13 est terminée.
- **#1 est traitée** : le référent fiscal a confirmé en fiche 3.5 la règle « jour coté le plus proche, y compris en avant », `rechercheCotation()` recherche désormais dans les deux sens avec départage sur le jour suivant en cas d'égalité, et les fixtures et tests concernés sont à jour (`v0.5.0-beta.14`).
...
- [#14 — Référentiel IR, CEHR, CDHR, PFU et PS](https://github.com/ARBE-Avocat/Simulateurs-fiscaux/issues/14) — **réalisée**
...
3. Livrée dans `v0.5.0-beta.12` : l'IFI utilise désormais `ChangeReferentiel.tauxALaDateMultiple` pour ses 25 devises — même mécanisme que la plus-value immobilière, un seul chargement du dépôt pour toutes les devises en repli plutôt qu'un par devise. `exchangerate-api.com` et le repli non daté `FX_FALLBACK` sont retirés du dépôt. La BCE seule couvre ce que faisait ce service tiers : une requête sur une fenêtre de dix jours ne renvoie pas d'erreur pour « aujourd'hui » non encore publié (~16h), elle rend simplement le dernier jour disponible — vérifié sur l'API réelle avant l'heure de publication.

**#13 est donc terminée.**

Conséquence visible : la conversion en devise n'est plus instantanée dans aucun des deux simulateurs, elle demande un aller-retour réseau (moins d'une seconde en pratique) ; l'IFI affiche « Chargement des taux de change en cours… » pendant ce court délai plutôt que d'afficher un résultat construit avec des devises non converties. Aucun montant final ne change une fois le taux résolu — preuve par des scénarios rejoués avec la vraie donnée de `data/change/` ou des taux injectés directement, puis en navigateur réel avec un vrai appel à la BCE, y compris le cas où l'appel échoue et celui d'une date tombant un jour non coté.

**Limite découverte en le vérifiant, pas un défaut** : `data/change/` ne couvre que les dates déjà extraites. Pour une date très récente si la BCE est aussi indisponible, ni l'une ni l'autre source ne répond, et l'écran l'affiche honnêtement plutôt que d'inventer un chiffre. Voir §7 bis.

**Défaut trouvé et corrigé au passage, hors devises** : `computePatrimoineBrut()` de l'IFI traitait un taux non résolu comme s'il valait 0 (silencieux), sous-évaluant le patrimoine taxable pendant le bref délai réseau. Il rend désormais `null` explicitement, et `compute()` affiche un état de chargement plutôt qu'un montant construit sur une donnée manquante. Une même relecture côté plus-value immobilière n'a rien trouvé d'équivalent atteignable : un garde-fou existant y bloque déjà l'affichage avant que le cas puisse se produire.

**#1 est tranchée et corrigée.** Le référent fiscal a répondu en fiche 3.5 le 28 août 2026 : c'est le jour coté le plus proche qui fait foi, y compris en avançant dans le temps, avec départage sur le jour suivant en cas d'égalité entre un jour antérieur et un jour postérieur. `rechercheCotation()` cherche désormais dans les deux sens au lieu de ne remonter que vers le passé ; l'effet touche 568 des 3 375 cas de l'échantillon de test, essentiellement des dimanches. Livré dans `v0.5.0-beta.14`.

**La vague C est terminée** : #13 et #1 sont toutes deux closes.
